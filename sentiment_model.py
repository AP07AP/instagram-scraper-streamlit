...
# -------------------------------------------
# rules_dict (UNCHANGED – copied exactly same)
# -------------------------------------------
rules_dict={
  "phonetic_mappings": {
...
# -------------------------------------------------
# MuRIL Sentiment Model (UNCHANGED)
# -------------------------------------------------
...
# ======================================================================
# ⭐ UPDATED analyze_comments — supports Raw_Comment automatically
# ======================================================================
def analyze_comments(df: pd.DataFrame) -> pd.DataFrame:
    """
    Automatically run sentiment analysis on:
        - Raw_Comment (preferred)
        - OR Comments (fallback)
    """

    # === Column auto-detection ===
    if "Raw_Comment" in df.columns:
        target_col = "Raw_Comment"
    elif "Comments" in df.columns:
        target_col = "Comments"
    else:
        raise ValueError("❌ No valid text column found: expected 'Raw_Comment' or 'Comments'.")

    # Backup original text
    original_text = df[target_col].copy()

    # Clean emojis for model input
    temp_text = df[target_col].fillna("").astype(str).apply(remove_emojis).str.strip()
    ...
    for text in tqdm(temp_text, desc="Analyzing Sentiments", disable=True):
    ...
    # Restore original unmodified text
    df[target_col] = original_text
